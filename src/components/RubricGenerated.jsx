...
var CRITERION = [
  {
    ...
    descripters: [
      ...
      {
        title_desc:
          "Ninguna de las lineas que vinvulan los componentes en el diagrama UML",
        value: 33,
    ...
    descripters: [
      {
        ...
        contextB:
          "Tienen declarados los tres componentes(nombre, atributos y operaciones)",
      },
      {
        title_desc:
          "El uso de la composicion esta siempre acorde con la definicion de UML en cuanto a que",
        value: 20,
      ...
      {
        title_desc:
          "El uso de la agregacion esta siempre acorde con la definicion de UML en cuanto a que",
        value: 20,
      ...
      {
        title_desc:
          "El uso de la asociacion esta siempre acorde con la definicion de UML en cuanto a que",
        value: 20,
      ...
      {
        title_desc:
          "El uso de la generalizacion esta siempre acorde con la definicion de UML en cuanto a que",
        value: 20,
        contextA: "El simbolo utilizado es una flecha de triangulo blanco",
        contextB:
          "Los atributos declarados en un nivel de la jerarquia no esta duplicado en un nivel mas bajo",
      },
    ],
  },
  {
    ...
    async function loadData() {
      try {
        ...
        const response = await axios.get(
          "http://localhost:8000/api/rubrica/" + id
        );
        setRubricas(response.data.name);
        const CRITERION = response.data.criterions;
        CRITERION.forEach((criterion, index) => {
          ...
          criterion.descripters.forEach((descripter, index) => {
            const newDescripter = criterio.createDescripter(
              descripter.title_desc
            );
            newDescripter.result = descripter.result;
            ...
            newDescripter.approveA = descripter.approveA;
            newDescripter.approveB = descripter.approveB;
          });
        });
        setLoading(false);
      } catch (error) {
